					...
					<div id="content">
						<div id="bbs_wrap">
							<div class="board_view">
								<dl class="tit_view">
									<dt>제목</dt>
									<dd id="titleTemp" contenteditable="false">${boardDto.title}</dd>
								</dl>
								<dl class="info_view2">
									<dt class="writer">작성자</dt>
									<dd class="writer">${boardDto.writer }</dd>
									<dt>작성일</dt>
									<dd>
										<%-- ${map.boardDto.day } --%>
									</dd>
										...
										<div class="fR">
											<form action="/board/edit" method="post"
												...
												<input type="hidden" id="user_id" name="user_id"
													id="user_id" value="<%=id%>"> <input
													type="hidden" id="bid" name="bid" value="${boardDto.id }">
												<input type="hidden" id="title" name="title"
													value="${boardDto.title }"> <input type="hidden"
													id="bcontents" name="bcontents"
								...
								<c:if test="${not empty id }">
									<div class="btn_all">
										<span class="bbtn"><a
											onclick="replyView(${map.boardDto.bid})">답글달기</a></span>
									</div>
								</c:if>
								...
								<div class="bbs_reply">
										...
										<c:forEach var="cdto" items="${cmap.clist }">
											<form class="commentBox" method="post"
												...
												<input type="hidden" name="recontent"
													value="${cdto.recontent }"> <strong>${cdto.user_id }</strong>
												<span class="date">${cdto.cdate } <%-- 로그인된 아이디와 작성자가 같을 경우 --%>
													<c:if test="${id eq cdto.user_id }">
														<input type="hidden" id="commentHidden"
															onclick="commentCRUD('2',this.form)" value="수정완료">
														<input type="button" id="commentToggle"
															onclick="Edit(this.form)" value="수정">
														<input type="button" onclick="commentCRUD('3',this.form)"
															value="삭제">
													</c:if>
												</span>
												<p class="reply_cont">
													<input class="insert" type="text" id="contentTemp"
														value="${cdto.recontent }" readonly>
												</p>
											</form>
										</c:forEach>
									</div>
									...
									<div class="comment">
										<div class="comment_inp">
											<form action="/board/CommentCheck" method="post"
												name="commentForm">

												<input type="hidden" name="user_id" id="user_id"
													value="<%=id%>"> <input type="hidden" name="bid"
													id="bid" value="${map.boardDto.bid }">

												<textarea rows="0" cols="0" maxlength="2000"
													name="recontent" id="" placeholder="댓글을 입력하세요."></textarea>
												<span class="bbtn_input"> <input type="button"
													onclick="commentCRUD('1',this.form)" value="등록"></span>
								...
								<div class="btn_all">
									...
									<div class="fL">
										...
										<span class="bbtn"><a id="updateBtn" onclick="editToggle()">수정</a></span> 
										<span class="bbtn"><a onclick="DeleteCheck(this.form)" title="삭제(질문, 답변 게시판)">삭제</a></span>
									</div>
									</c:if>
									<div class="fR">
										<span class="bbtn"><a
											onclick="location.href='/board/list?category=${map.category}&page=${map.page }&search=${map.search }'"
											title="일반게시판">목록</a></span>
									</div>
								</div>
